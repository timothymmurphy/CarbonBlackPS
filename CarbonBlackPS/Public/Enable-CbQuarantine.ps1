<#
    .SYNOPSIS
    Quarantines a device within Carbon Black.
    ...
    .PARAMETER SensorID
    Sensor ID to quarantine. This is the safest method and ensures you target the correct device.

    .PARAMETER ComputerName
    Name of the device to quarantine. Use at your own risk, if there are multiple devices named similar to the value specified you may target a device you don't intend to.
    Highly recommend using in conjunction with -WhatIf first to ensure that the intended device is targeted.
    
    .EXAMPLE
    Enable-CbQuarantine -SensorID 12345678

    .EXAMPLE
    Enable-CbQuarantine -ComputerName "tim-pc"

#>

function Enable-CbQuarantine {
    ...
    if ($PSCmdlet.ShouldProcess($device.name, "quarantine")) {
