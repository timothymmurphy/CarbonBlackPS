<#
    .SYNOPSIS
    Enables background scan on a device within Carbon Black.
    ...
    .PARAMETER SensorID
    Sensor ID of device to begin background scan on. This is the safest method and ensures you target the correct device.

    .PARAMETER ComputerName
    Name of the device to begin background scan on. Use at your own risk, if there are multiple devices named similar to the value specified you may target a device you don't intend to.
    Highly recommend using in conjunction with -WhatIf first to ensure that the intended device is targeted.
    
    .EXAMPLE
    Enable-CbBackgroundScan -SensorID 12345678

    .EXAMPLE
    Enable-CbBackgroundScan -ComputerName "tim-pc"

#>

function Enable-CbBackgroundScan {
    ...
    if ($PSCmdlet.ShouldProcess($device.name, "begin background scan")) {
